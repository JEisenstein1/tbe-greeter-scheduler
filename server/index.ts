...
function sanitizeUserMessage(rawMessage: unknown) {
  if (typeof rawMessage !== 'string') return { ok: false, error: 'message required' };
  const normalized = rawMessage.normalize('NFKC').trim();
  if (!normalized) return { ok: false, error: 'message required' };
  if (CONTROL_CHAR_RE.test(normalized)) return { ok: false, error: 'message contains unsupported control characters' };
  if (normalized.length > MAX_MESSAGE_LENGTH) return { ok: false, error: `message exceeds ${MAX_MESSAGE_LENGTH} characters` };
  return { ok: true, message: normalized };
}

function classifyMessageScope(message: string) {
  const lower = message.toLowerCase();
  if (DISALLOWED_PATTERNS.some(pattern => pattern.test(lower))) return { allowed: false, reason: 'blocked_pattern' };
  if (ALLOWED_PATTERNS.some(pattern => pattern.test(lower))) return { allowed: true, reason: 'allowed_pattern' };
  return { allowed: false, reason: 'off_topic' };
}

function buildSystemPrompt(role: string, user: any, services: any[] = []) {
  const today = new Date().toISOString().slice(0, 10);
  const svcLines = services.map((s: any) => {
    const slots = Array.isArray(s.slots) ? s.slots : [];
    const slotSummary = slots.map((sl: any) => `    - [${sl.id}] ${sl.role}${sl.timeSlot ? ` (${sl.timeSlot})` : ''}: ${sl.volunteer ?? 'OPEN'}`).join('\n');
    const open = slots.filter((sl: any) => !sl.volunteer).length;
    const filled = slots.filter((sl: any) => sl.volunteer).length;
...
Only call create_service when the admin clearly intends to add a service. Confirm details conversationally before calling the tool if key information is missing.`;
  ...
  const mySlots = user ? services.flatMap((s: any) => (Array.isArray(s.slots) ? s.slots : []).filter((sl: any) => sl.volunteer === user.name).map((sl: any) => ({ svc: s, slot: sl }))) : [];
  const mySlotLines = mySlots.length ? mySlots.map(({ svc, slot }: any) => `  - [svcId: ${svc.id}, slotId: ${slot.id}] ${svc.type} on ${svc.date} · ${slot.role}${slot.timeSlot ? ` (${slot.timeSlot})` : ''}`).join('\n') : '  (none yet)';
  const openSlots = services.flatMap((s: any) => (Array.isArray(s.slots) ? s.slots : []).filter((sl: any) => !sl.volunteer).map((sl: any) => ({ svc: s, slot: sl })));
  const openLines = openSlots.length ? openSlots.slice(0, 12).map(({ svc, slot }: any) => `  - [svcId: ${svc.id}, slotId: ${slot.id}] ${svc.type} on ${svc.date} · ${slot.role}${slot.timeSlot ? ` (${slot.timeSlot})` : ''}`).join('\n') : '  (no open slots)';
...
function parseToolArguments(args: unknown) {
  if (!args) return {};
  if (typeof args === 'object') return args as Record<string, unknown>;
  try { return JSON.parse(String(args)); } catch { return {}; }
}

function extractOpenRouterResponse(data: any) {
  const message = data?.choices?.[0]?.message ?? {};
  const text = typeof message.content === 'string' ? message.content : '';
  const actions: Array<Record<string, unknown>> = [];
  for (const toolCall of message.tool_calls ?? []) {
    const name = toolCall.function?.name;
    const input: any = parseToolArguments(toolCall.function?.arguments);
...
function actionIntentPermitsTools(message: string, actionName: unknown) {
  const lower = message.toLowerCase();
...
app.post('/api/chat', async (req, res) => {
  ...
  if (!apiKey) { res.status(500).json({ error: 'OPENROUTER_API_KEY is not set' }); return; }

  const body = req.body;
  const sanitized = sanitizeUserMessage(body?.message);
  if (!sanitized.ok) { res.status(400).json({ error: sanitized.error }); return; }

  const scope = classifyMessageScope(sanitized.message!);
  if (!scope.allowed) { res.json({ text: REFUSAL_TEXT, actions: [] }); return; }
  ...
  try {
    ...
    if (!openRouterRes.ok) {
      ...
      res.status(502).json({ error: `OpenRouter API request failed (${openRouterRes.status})` });
      return;
    }
