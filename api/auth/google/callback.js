...
async function upsertUser(user) {
  ...
  await sql`
    ...
    VALUES (${user.googleSub}, ${user.email}, ${user.name}, ${user.role})
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!stateCookie?.state || req.query.state !== stateCookie.state) {
    ...
    res.setHeader('Set-Cookie', [
      serializeCookie(COOKIE_NAME, createSessionCookie(user), { maxAge: 60 * 60 * 8 }),
